...
// collapsed store as DesktopSidebar directly rather than threading a
// `collapsed` prop through the server component that fetches notifications
// (layout.tsx can't know client-only collapse state at render time).
export function NotificationsDropdown({ notifications }: { notifications: NotificationItem[] }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
          collapsed ? (
            ...
            <Button variant="ghost" className="text-muted-foreground w-full justify-start gap-3 px-3 font-normal">
              <Bell className="size-4 shrink-0" />
              <span className="flex-1 text-left text-sm">Notifications</span>
              {notifications.length > 0 && (
                <span className="bg-destructive text-destructive-foreground flex size-5 shrink-0 items-center justify-center rounded-full text-[10px] font-medium">
                  {notifications.length > 9 ? "9+" : notifications.length}
                </span>
              )}
            </Button>
      ...
      />
      <DropdownMenuContent align="start" side={collapsed ? "right" : "top"} className="w-80">
